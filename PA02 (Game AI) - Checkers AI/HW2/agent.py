...
# For node ordering:
#   pawn's value = row number
#   king's value = row number + 2
#   where row number is between 0 and 7 for 'b' and the reverse for 'r'
# For game playing:
#   pawn's value = 1
#   king's value = 3
#########################################################################
...
def compute_utility(state, color):
    ...
    for row in state.board:
        for cell in row:
            ...
            elif cell == 'R': r_count += 2
            elif cell == 'b': b_count += 1
            elif cell == 'B': b_count += 2
...
def compute_heuristic(state, color): 
    ...
    for i, row in enumerate(state.board):
        for cell in row:
            if order_flag:  # heuristic for node ordering
                if cell == '.': continue
                elif cell == 'r': r_count += 7 - i
                elif cell == 'R': r_count += 9 - i
                elif cell == 'b': b_count += i
                elif cell == 'B': b_count += 2 + i
            else:  # heuristic for game playing
                if cell == '.': continue
                elif cell == 'r': r_count += 1
